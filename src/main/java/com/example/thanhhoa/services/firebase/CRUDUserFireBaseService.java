...
public class CRUDUserFireBaseService {
    ...
    public static String createUser(UserFCMToken userFCMToken){
        Firestore dbFirestore = FirestoreClient.getFirestore();
        List<ApiFuture<WriteResult>> futures = new ArrayList<>();
        CollectionReference users = dbFirestore.collection("Users");
        futures.add(users.document(userFCMToken.getUsername()).set(userFCMToken)
        );
        return "Đã tạo";
    }
    ...
    public static String updateUser(UserFCMToken userFCMToken) throws InterruptedException, ExecutionException {
        Firestore dbFirestore = FirestoreClient.getFirestore();
        ApiFuture<WriteResult> collectionsApiFuture = dbFirestore.collection("Users").document(userFCMToken.getUsername()).set(userFCMToken);
        return collectionsApiFuture.get().getUpdateTime().toString();
    }

    public static String deleteUser(UserFCMToken userFCMToken) {
        Firestore dbFirestore = FirestoreClient.getFirestore();
        dbFirestore.collection("Users").document(userFCMToken.getUsername()).delete();
        return "Đã xóa.";
    }
